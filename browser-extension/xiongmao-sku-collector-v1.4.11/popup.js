...
async function copyTencent() {
  ...
  try {
    ...
    if (imageRows.length === 0) {
      setStatus("没有可用的图片，复制纯表格。", "warn");
    }

    // 2. 上传到公共图床（拿到 https:// URL 给腾讯文档用，data: URL 它会过滤掉）
    setStatus(`正在上传 ${imageRows.length} 张图到公共图床（确保腾讯文档/同事能看）...`, "");
    const uploadResults = await uploadImagesBatch(blobMap);
    const successCount = Object.values(uploadResults).filter((v) => /^https?:\/\//.test(v || "")).length;
    const failedCount = imageRows.length - successCount;

    if (successCount === 0) {
      setStatus("图床上传全部失败。腾讯文档粘贴需要公网图片，建议先用'下载 Excel（内嵌图）'。", "bad");
      setBusy(false);
      return;
    }

    // 3. 构造剪贴板 HTML：图片用 https URL，失败的留文字提示
    const imageMap = {};
    for (const [i, url] of Object.entries(uploadResults)) {
    ...
    if (navigator.clipboard && window.ClipboardItem) {
      ...
      setStatus(
        `已写入剪贴板：${successCount} 张图已上传到公共图床${failedCount ? `（${failedCount} 张失败）` : ""}。到腾讯文档/微信表格直接 Ctrl+V 粘贴即可。`,
...
async function fetchImageWithTimeout(url, timeoutMs) {
  ...
  try {
    const res = await fetch(url, { credentials: "include", signal: controller.signal, cache: "force-cache" });
