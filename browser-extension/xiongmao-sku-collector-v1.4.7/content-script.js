(() => {
  ...
  const DASH_CHARS = "\\-‐‑‒–—―";
  const SHIPMENT_NUMBER_PATTERN = new RegExp(`\\b\\d{8,12}[${DASH_CHARS}]\\d{3,5}[${DASH_CHARS}]\\d{1,4}\\b`);
  const PRICE_PATTERN = /(?:[₽￥¥$]\s*)?\d[\d\s]*[.,]\d{2}\s*(?:₽|￥|¥|руб\.?|RUB|\$)?/i;
  ...
  function isOrderDataRow(rowElement, headers = []) {
    ...
    if (headers.includes("货件编号")) {
      ...
      const hasPrice = cells.some((cell) => PRICE_PATTERN.test(compactText(cell)));
  ...
  function parseOrderRow(row, cells) {
    ...
    for (const cell of cells) {
      ...
      const img = cell.querySelector("img[src], img[data-src]");
      if (!img) continue;
      const text = compactText(cell);
      ...
      if (textLength <= 80) {
        row["照片"] = img.src || img.getAttribute("data-src") || "";
        visited.add(cell);
        break;
      }
    }
    ...
    for (const cell of cells) {
      ...
      const isPrice = PRICE_PATTERN.test(text);
  ...
  function parseOrderQuantityCell(cell, soft = false) {
    ...
    const imgEl = cell.querySelector?.("img[src], img[data-src]");
    const image = (imgEl?.src || imgEl?.getAttribute("data-src") || "");
    const parsed = parseOrderQuantityLines(cell) || parseOrderQuantityText(text);
  ...
  function parseOrderQuantityText(text) {
    const normalized = cleanOrderProductText(compactText(text));
    ...
    let name = remainder
      .replace(/\s*(?:[₽￥¥$]\s*)?\d[\d\s.,]*\s*(?:₽|￥|¥|руб|RUB|\$)?\s*/gi, " ")
      .replace(/\s{2,}/g, " ")
      .trim();
  ...
  function parseOrderQuantityLines(cell) {
    const lines = getTextLines(cell).map(cleanOrderProductText).filter(Boolean);
    if (!lines.length) return null;

    const UNIT = "(?:[个件双套条瓶盒只袋包箱架捆份盒台套组款枚块张片把卷桶]|шт\\.?|пар\\.?|уп\\.?|компл\\.?)";
    const lineIndex = lines.findIndex((line) => new RegExp(`\\d+(?:\\.\\d+)?\\s*${UNIT}`, "i").test(line));
    if (lineIndex < 0) return null;

    const line = lines[lineIndex];
    const quantityMatch = line.match(new RegExp(`(\\d+(?:\\.\\d+)?\\s*${UNIT})`, "i"));
    if (!quantityMatch) return null;

    let remainder = line.slice((quantityMatch.index || 0) + quantityMatch[0].length).trim();
    const skuMatch = remainder.match(/^([A-Za-z0-9][A-Za-z0-9_\-]*?)(?=[A-Z][a-z]|\s|[^\x00-\x7F]|$)/);
    const sku = skuMatch ? skuMatch[1] : "";
    remainder = sku ? remainder.slice(sku.length).trim() : remainder;

    const nameParts = [];
    if (remainder) nameParts.push(remainder);
    lines.slice(lineIndex + 1).forEach((nextLine) => {
      if (!PRICE_PATTERN.test(nextLine) && !extractShipmentNumber(nextLine)) nameParts.push(nextLine);
    });

    return {
      "数量": quantityMatch[1].replace(/\s+/g, ""),
      "货号": sku,
      "商品名称": nameParts.join(" ").trim()
    };
  }

  function cleanOrderProductText(value) {
    return cleanCellText(value)
      .replace(/(复制|Copy|Copied|Скопировать|Скопировано)/gi, " ")
      .replace(/\s+/g, " ")
      .trim();
  }
  ...
  function normalizeOrderRow(row) {
    ...
    if (!row["照片"]) {
      const img = [...document.querySelectorAll(`[data-row-key]`)]
        .find((el) => compactText(el).includes(row["货件编号"] || ""))?.querySelector("img[src]");
      if (img) row["照片"] = img.src;
    }

    // 日期字段保留小时分钟（例如 "28 Jun 22:00"、"4 Jul 至07:00"）
    for (const key of ["发送日期", "已接收", "不逾期"]) {
      if (row[key]) {
        row[key] = cleanCellText(row[key]);
      }
    }
