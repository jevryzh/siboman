...
const copyImageModeInput = document.getElementById("copyImageMode");
const qiniuAccessKeyInput = document.getElementById("qiniuAccessKey");
const qiniuSecretKeyInput = document.getElementById("qiniuSecretKey");
const qiniuBucketInput = document.getElementById("qiniuBucket");
const qiniuDomainInput = document.getElementById("qiniuDomain");
const qiniuUploadHostInput = document.getElementById("qiniuUploadHost");
const saveQiniuBtn = document.getElementById("saveQiniuBtn");
const progressWrap = document.getElementById("progressWrap");
const progressLabel = document.getElementById("progressLabel");
const progressPct = document.getElementById("progressPct");
const progressBar = document.getElementById("progressBar");

const DEFAULT_QINIU_SETTINGS = {
  copyImageMode: "qiniu",
  accessKey: "",
  secretKey: "",
  bucket: "rwz-lease-resource",
  domain: "https://source.renwz.cn",
...
async function init() {
  ...
  await loadQiniuSettingsIntoForm();
  try {
...
saveQiniuBtn?.addEventListener("click", saveQiniuSettingsFromForm);
copyImageModeInput?.addEventListener("change", saveQiniuSettingsFromForm);

async function loadQiniuSettingsIntoForm() {
  const settings = await getStoredQiniuSettings();
  if (copyImageModeInput) copyImageModeInput.value = settings.copyImageMode;
  if (qiniuAccessKeyInput) qiniuAccessKeyInput.value = settings.accessKey;
  if (qiniuBucketInput) qiniuBucketInput.value = settings.bucket;
  if (qiniuDomainInput) qiniuDomainInput.value = settings.domain;
  if (qiniuUploadHostInput) qiniuUploadHostInput.value = settings.uploadHost;
  if (qiniuSecretKeyInput) {
    qiniuSecretKeyInput.value = "";
    qiniuSecretKeyInput.placeholder = settings.secretKey ? "已保存；不修改可留空" : "粘贴 SK";
  }
}

async function saveQiniuSettingsFromForm() {
  const existing = await getStoredQiniuSettings();
  const settings = normalizeQiniuSettings({
    copyImageMode: copyImageModeInput?.value || DEFAULT_QINIU_SETTINGS.copyImageMode,
    accessKey: qiniuAccessKeyInput?.value || "",
    secretKey: qiniuSecretKeyInput?.value || existing.secretKey || "",
    bucket: qiniuBucketInput?.value || "",
    domain: qiniuDomainInput?.value || "",
    uploadHost: qiniuUploadHostInput?.value || ""
  });

  await chrome.storage.local.set({ qiniuSettings: settings });
  await loadQiniuSettingsIntoForm();
  setStatus(
    hasQiniuSettings(settings)
      ? "七牛 CDN 设置已保存。复制到腾讯文档时会先上传图片到你的 CDN。"
      : "七牛设置已保存，但 AK/SK/Bucket/CDN 域名还没填完整；复制时会用本地小图兜底。",
    hasQiniuSettings(settings) ? "good" : "warn"
  );
}

async function getStoredQiniuSettings() {
  try {
    const stored = await chrome.storage.local.get("qiniuSettings");
    return normalizeQiniuSettings({
      ...DEFAULT_QINIU_SETTINGS,
      ...(stored.qiniuSettings || {})
    });
  } catch {
    return normalizeQiniuSettings(DEFAULT_QINIU_SETTINGS);
  }
}

function normalizeQiniuSettings(settings) {
  return {
    copyImageMode: settings.copyImageMode === "inline" ? "inline" : "qiniu",
    accessKey: String(settings.accessKey || "").trim(),
    secretKey: String(settings.secretKey || "").trim(),
    bucket: String(settings.bucket || DEFAULT_QINIU_SETTINGS.bucket).trim(),
    domain: normalizeUrl(settings.domain, DEFAULT_QINIU_SETTINGS.domain),
    uploadHost: normalizeUrl(settings.uploadHost, DEFAULT_QINIU_SETTINGS.uploadHost)
  };
}
...
function hasQiniuSettings(settings) {
  return Boolean(settings?.accessKey && settings?.secretKey && settings?.bucket && settings?.domain && settings?.uploadHost);
}
...
async function copyTencent() {
  ...
  setBusy(true);
  updateProgress("准备处理图片", 0);
  setStatus("正在准备复制到腾讯文档...", "");

  try {
    const settings = await getStoredQiniuSettings();
    const imageWanted = latestResult.rows.filter((row) => row["商品图片"] || row["照片"]).length;
    const useQiniu = settings.copyImageMode === "qiniu" && hasQiniuSettings(settings);

    if (settings.copyImageMode === "qiniu" && !useQiniu) {
      setStatus("七牛设置还没填完整，正在改用本地内嵌小图。", "warn");
    }

    const blobMap = await fetchImagesAsBlobs(latestResult.rows, {
      onProgress: ({ current, total }) => {
        const percent = total ? (current / total) * 35 : 35;
        updateProgress(`下载图片 ${current}/${total}`, percent);
      }
    });
    ...
    if (useQiniu && imageRows.length) {
      const uploadResult = await uploadImagesToQiniu(blobMap, settings, {
        onProgress: ({ current, total, itemPercent }) => {
          const percent = 35 + ((Math.max(0, current - 1) + itemPercent) / Math.max(1, total)) * 58;
          updateProgress(`上传 CDN ${current}/${total}`, percent);
        }
      });
      imageMap = uploadResult.imageMap;
      uploadedCount = Object.values(imageMap).filter(Boolean).length;
      uploadFailedCount = uploadResult.failedCount;

      const fallbackBlobs = pickMissingBlobMap(blobMap, imageMap);
      if (Object.keys(fallbackBlobs).length) {
        const fallbackMap = await buildClipboardImageMap(fallbackBlobs, {
          onProgress: ({ current, total }) => {
            const percent = 93 + (current / Math.max(1, total)) * 4;
            updateProgress(`压缩失败图片 ${current}/${total}`, percent);
          }
        });
        inlineCount = Object.values(fallbackMap).filter(Boolean).length;
        imageMap = { ...imageMap, ...fallbackMap };
      }
    } else {
      imageMap = await buildClipboardImageMap(blobMap, {
        onProgress: ({ current, total }) => {
          const percent = 35 + (current / Math.max(1, total)) * 60;
          updateProgress(`压缩图片 ${current}/${total}`, percent);
        }
      });
      inlineCount = Object.values(imageMap).filter(Boolean).length;
    }

    const readyImageCount = Object.values(imageMap).filter(Boolean).length;
    const failedDownloadCount = Math.max(0, imageWanted - imageRows.length);
    const failedCount = Math.max(0, imageWanted - readyImageCount);

    updateProgress("写入剪贴板", 98);
    ...
    if (navigator.clipboard && window.ClipboardItem) {
      ...
      updateProgress("已写入剪贴板", 100);
      setStatus(
        !imageWanted
          ? `已写入剪贴板：${latestResult.rows.length} 条数据，本页没有需要处理的图片。到腾讯文档直接 Ctrl+V。`
          : readyImageCount
          ? `已写入剪贴板：${uploadedCount ? `${uploadedCount} 张 CDN 图` : ""}${uploadedCount && inlineCount ? "，" : ""}${inlineCount ? `${inlineCount} 张本地小图` : ""}${failedCount ? `，${failedCount} 张保留原链接` : ""}。到腾讯文档直接 Ctrl+V。`
          : "已写入剪贴板，但没有图片成功处理；腾讯文档若仍不显示，请用'下载 Excel（内嵌图）'导入。",
        failedCount ? "warn" : "good"
      );
    } else {
      await navigator.clipboard.writeText(csvText);
      updateProgress("已复制 CSV", 100);
      setStatus("当前浏览器不支持 HTML 剪贴板，已回退到 CSV。", "warn");
    }
    if (uploadFailedCount || failedDownloadCount) {
      console.warn("[xiongmao] 图片处理未全部成功", { uploadFailedCount, failedDownloadCount });
    }
  } catch (error) {
    updateProgress("复制失败", 100);
...
async function buildClipboardImageMap(blobMap, options = {}) {
  const imageMap = {};
  const entries = Object.entries(blobMap).filter(([, item]) => item?.blob);
...
function pickMissingBlobMap(blobMap, imageMap) {
  const result = {};
  for (const [index, item] of Object.entries(blobMap)) {
    if (item?.blob && !imageMap[index]) result[index] = item;
  }
  return result;
}

async function uploadImagesToQiniu(blobMap, settings, options = {}) {
  const entries = Object.entries(blobMap).filter(([, item]) => item?.blob);
  const imageMap = {};
  ...
  for (let i = 0; i < entries.length; i += 1) {
    const [index, item] = entries[i];
    const cachedUrl = item.url ? uploadedBySource.get(item.url) : "";
    if (cachedUrl) {
      imageMap[index] = cachedUrl;
      options.onProgress?.({ current: i + 1, total: entries.length, itemPercent: 1 });
      continue;
    }

    try {
      const key = makeQiniuObjectKey(item.filename, index);
      const publicUrl = await uploadBlobToQiniu(item.blob, item.filename, key, settings, ({ loaded, total }) => {
        const safeTotal = total || item.blob.size || 1;
        options.onProgress?.({
          current: i + 1,
          total: entries.length,
          itemPercent: Math.min(1, loaded / safeTotal)
        });
      });
      imageMap[index] = publicUrl;
      if (item.url) uploadedBySource.set(item.url, publicUrl);
    } catch (error) {
      failedCount += 1;
      imageMap[index] = null;
      console.warn("[xiongmao] 七牛上传失败", item.filename, error);
    }
    options.onProgress?.({ current: i + 1, total: entries.length, itemPercent: 1 });
  }

  return { imageMap, failedCount };
}

async function uploadBlobToQiniu(blob, filename, key, settings, onProgress) {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.open("POST", settings.uploadHost, true);
    xhr.timeout = 45000;
    ...
    xhr.onload = () => {
      if (xhr.status < 200 || xhr.status >= 300) {
        reject(new Error(`七牛上传失败 HTTP ${xhr.status}: ${xhr.responseText || ""}`.trim()));
        return;
      }
      resolve(buildQiniuPublicUrl(settings.domain, key));
    };
    xhr.onerror = () => reject(new Error("七牛上传网络失败"));
    xhr.ontimeout = () => reject(new Error("七牛上传超时"));
    xhr.send(form);
  });
}

async function createQiniuUploadToken(settings, key) {
  ...
  const encodedSign = await hmacSha1Base64Url(settings.secretKey, encodedPolicy);
...
function makeQiniuObjectKey(filename, index) {
  const now = new Date();
  const date = [
    now.getFullYear(),
  ...
  const safeName = String(filename || `image_${index}.jpg`)
    .replace(/[^\w.-]/g, "_")
    .slice(0, 80);
  const suffix = Math.random().toString(36).slice(2, 8);
  return `xiongmao/${date}/${Date.now()}_${index}_${suffix}_${safeName}`;
}

function buildQiniuPublicUrl(domain, key) {
  const safeKey = String(key).split("/").map(encodeURIComponent).join("/");
  return `${domain}/${safeKey}`;
}

// 构造可直接粘贴到腾讯文档的 HTML 表格。图片可以是七牛 CDN URL，也可以是本地 data:image 兜底。
...
async function downloadXlsx() {
  ...
  try {
    ...
    setStatus(
      `Excel 已下载：${latestResult.rows.length} 条，内嵌 ${successCount} 张图；图片列同时写入 IMAGE 公式${failedCount ? `，${failedCount} 张下载失败保留公式/链接` : ""}。`,
...
// 下载每张图，返回 { index: { blob, filename, ext, url } }
async function fetchImagesAsBlobs(rows, options = {}) {
  const map = {};
  const seen = new Map();
  let counter = 0;
  let finished = 0;
  const total = rows.filter((row) => {
    const url = row["商品图片"] || row["照片"];
    return url && /^https?:/i.test(url);
  }).length;

  if (!total) {
    options.onProgress?.({ current: 0, total: 0 });
    return map;
  }
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      map[i] = { blob, filename, ext, url };
    } catch (e) {
      // 单张失败不阻断
      console.warn("[xiongmao] 图片下载失败", url, e);
    } finally {
      finished += 1;
      options.onProgress?.({ current: finished, total });
    }
  }
  return map;
}
  ...
  const body = rows
    .map((row, rowIndex) => {
      const values = columns.map((column, columnIndex) => {
        ...
        if (columnIndex === imageColumnIndex && value) {
          return {
            formula: `IMAGE("${excelFormulaString(value)}")`,
  ...
  const cells = values
    .map((value, index) => {
      ...
      if (value && typeof value === "object" && value.formula) {
        return `<c r="${ref}"><f>${escapeXml(value.formula)}</f></c>`;
      }
...
function setBusy(isBusy) {
  ...
  collectBtn.textContent = isBusy ? "处理中..." : "采集当前页";
  const hasRows = Boolean(latestResult?.rows?.length);
  [
    copyCsvBtn,
    downloadCsvBtn,
    downloadXlsxBtn,
    ...
    if (button) button.disabled = isBusy || !hasRows;
  });
}
...
function updateProgress(label, percent) {
  ...
  if (progressPct) progressPct.textContent = `${Math.round(safePercent)}%`;
...
function resetProgress() {
  ...
  if (progressLabel) progressLabel.textContent = "准备中";
