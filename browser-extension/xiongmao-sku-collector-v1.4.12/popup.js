...
async function copyTencent() {
  ...
  try {
    ...
    // 3. 构造剪贴板 HTML：优先用上传后的 https URL；上传失败时保留 Ozon 原始图片链接兜底
    const imageMap = {};
    ...
    if (navigator.clipboard && window.ClipboardItem) {
      ...
      if (successCount > 0) {
        setStatus(
          `已写入剪贴板：${successCount} 张图已上传到公共图床${failedCount ? `（${failedCount} 张使用原始链接兜底）` : ""}。到腾讯文档/微信表格直接 Ctrl+V 粘贴即可。`,
          failedCount && failedCount > successCount / 2 ? "warn" : "good"
        );
      } else {
        setStatus("已写入剪贴板：图床上传失败，已改用 Ozon 原始图片链接兜底。若腾讯文档仍不显示，请用'下载 Excel（内嵌图）'。", "warn");
      }
    } else {
