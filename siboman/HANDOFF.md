...
### 3.1 后端（`server.js` 单文件 6100+ 行）

| 层 | 用什么 | 版本 | 说明 |
|---|---|---|---|
| 语言 / 运行时 | Node.js（原生 JS + ESM） | ≥ 20（`.nvmrc` = 23.11.0） | 全部 `.js`，无 TypeScript |
| HTTP 框架 | **Express** | 4.19.2 | 单入口，未拆 router/controller |
| DB 客户端 | **pg**（node-postgres） | 8.16.5 | 一个 `Pool(DATABASE_URL)` |
| DB | **PostgreSQL** | — | 5 张表：`app_users` / `app_jobs` / `collect_items` / `order_notes` / `ai_image_records` |
| 浏览器自动化 | **Playwright** | 1.44.0 | 只在**本机 collector.js** 用；服务器 `DISABLE_SERVER_SCRAPER=true` |
| 压缩 / xlsx | **fflate** | 0.8.2 | 手写 XML + fflate 打包，不依赖 exceljs |
| AI - 文本 | **MiniMax-M3**（`/v1/chat/completions`） | — | 直接 fetch，无 SDK |
| AI - 图生图 | **MiniMax image-01 / image-01-live**（`/v1/image_generation`） | — | 单张约 $0.03，官方 `n` 上限 9 |
| 进程管理 | **systemd** | — | `ozon-app.service` |
| 反向代理 | **nginx** | — | `xm.renwz.cn` → 127.0.0.1:5177 |

**关键特点**：
- **无 ORM**：全 `db.query(sql, args)` 手写 SQL；DDL 集中在 `initDatabase()` @ line 295，全 `CREATE TABLE IF NOT EXISTS` 幂等
- **无中间件生态**（无 helmet / morgan / passport）：认证 100 行自撸，`AUTH_COOKIE` + `scrypt` hash（server.js:158-292）
- **单文件设计有意为之**：好 grep、AI 生成友好、一人维护心智负担低。行数超 8000 时再考虑按 `routes/*` `services/*` 拆分

### 3.2 前端（`public/` 目录）

| 层 | 用什么 | 说明 |
|---|---|---|
| 框架 | **无框架**（Vanilla JS） | 原生 ES6+ |
| 构建 | **无构建**（无 Vite / Webpack） | 浏览器直接加载 `.js` |
| 模板 | **模板字符串** | `` root.innerHTML = `...` `` |
| 路由 | **hash 路由**（`#/dashboard`） | 自撸 15 行 `applyRoute()`（app.js:94） |
| 状态 | **一个全局 `state = {}`** | 无 Redux / Pinia |
| 样式 | **原生 CSS** | `styles.css` + `app-shell.css`，无 Tailwind |
| HTTP | **原生 `fetch`** | 封装 `postJson()` / `getJson()` |
| 组件 | **`render*` 函数**（17+ 个） | 每页一个函数，返回 HTML 字符串 |

**关键特点**：
- **无 npm 依赖**：`public/` 里没 `node_modules`，浏览器直接跑
- **无 build 步骤**：改 `.js` → Cmd+R 立即见效
- **无图表库**（Chart.js / ECharts 未引；09 数据大屏会引一个）
- 单文件 1900+ 行 `app.js`；行数超 3000 时按 `pages/*.js` 拆

### 3.3 采集端（`collector.js` 301 行）

- 语言：Node.js + Playwright
- 运行：本机 Mac `screen -dmS ozon-collector node collector.js`
- 通讯：轮询服务器 `/api/worker/jobs/next` 领任务，`/progress` `/complete` 汇报
- 持久化 profile 保 1688 登录 cookie
...
| **上架** | 内部 `/api/seller/products/import` | 调 `/v3/product/import` |
...
2. **【高】** 上架页下一步要接 Ozon 类目属性模板：按 `category_id/type_id` 拉必填属性，并把 JSON 编辑改成表单填写
3. **【中】** 自动上架任务流：采集箱多选 → 批量预检 → 利润计算 → 批量提交 → 批量同步 task 状态
4. **【中】** AI 商品套图 → 一键带入上架页多图，当前已支持上架页多图预览/设主图/提交
5. **【中】** 同步库存的"bulk 模式"（一次拉所有库存 → 表格编辑 → 批量提交，目前是手勾）
6. **【低】** HTTPS（`xm.renwz.cn` 现在是 HTTP）
7. **【低】** 账号管理 UI
8. **【低】** 任务断点续传
