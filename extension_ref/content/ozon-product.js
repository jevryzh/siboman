(function() {
  ...
  function createActionBar() {
    ...
    const erpBtn = createActionButton(_ICONS.erp, '进入ERP', () => {
      window.open(`https://${globalThis.__JZ_BRAND__.webHost}`, '_blank');
    });
      ...
      if (needPhaseA) {
        ...
        phaseA()
          ...
          .finally(() => {
            ...
            if (full.length > _origVariantCount && !_followPanel.dataset.jzTouched) {
              // 发现新变体且用户未动 → 用全量变体重开(deferSourceFetch=false 正常拉源属性)
              _followPanel = _openFollowPanel(full, false);
            } else {
              // 不重开:把首次 defer 掉的源属性补上(对当前面板变体)
              try { _followPanel._startSourceFetch?.(); } catch {}
              if (full.length > _origVariantCount) {
                const badge = _followPanel.querySelector('[data-field="variant-badge"]');
